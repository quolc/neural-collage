...
def gen_images_with_condition(gen, z=None, c=0, n=500, batchsize=100):
    ims = []
    xp = gen.xp
    for i in range(0, n, batchsize):
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
...
def sample_reconstruction(enc, gen, dst, rows=10, samples=5, n_classes=1000, seed=0):
    """Visualization of rows*cols reconstruction result."""
    # classes = np.asarray(np.arange(rows) * (n_classes / rows), dtype=np.int32)
    ...
    def make_image(trainer=None):
        ...
        with chainer.using_config('train', False), chainer.using_config('enable_backprop', False):
            for i in range(rows):
                row = []
                x, gt, c = trainer.updater.get_batch(xp)
                if trainer.updater.input_size is not None:
                    size = trainer.updater.input_size
                    _x = []
                    for img in x.data.get():
                        _x.append(chainercv.transforms.resize(img, (size, size)))
                    _x = Variable(xp.asarray(_x))
                    if enc.n_classes != 0:
                        z = enc(_x, c)
                    else:
                        z = enc(_x)
                ...
                for j in range(samples):
                    row.append(gt.data[j].get())
                    row.append(x.data[j].get())
                    row.append(recon.data[j].get())
                xs.append(row)
        x = np.concatenate(xs, 0)

        _, _, h, w = x.shape
        x = x.reshape((rows, samples*3, 3, h, w))
        x = x.transpose(0, 3, 1, 4, 2)
        x = x.reshape((rows * h, samples*3 * w, 3))
        x = np.asarray(np.clip(x * 127.5 + 127.5, 0.0, 255.0), dtype=np.uint8)
...
def sample_reconstruction_auxab(enc, gen, dst, rows=8, cols=2, n_classes=1000, seed=0):
    """Visualization of rows*cols reconstruction result."""
    # classes = np.asarray(np.arange(rows) * (n_classes / rows), dtype=np.int32)
    ...
    def make_image(trainer=None):
        ...
        with chainer.using_config('train', False):
            ...
            for i in range(rows):
                row = []
                for j in range(cols):
                    if batchsize == 0 or cnt == batchsize - 1:
                        x, gt, c = trainer.updater.get_batch(xp)
                        batchsize = x.shape[0]
                        cnt = 0
                        ...
                        outs, _, out_noab, _, _ = gen(batchsize=len(z), z=z, y=c, gt=gt)

                    row.append(gt.data[cnt].get())
                    row.append(out_noab.data[cnt].get())
                    for k in range(gen.T + 1):
                        row.append(outs[k].data[cnt].get())
                    cnt += 1
                xs.append(row)
        x = np.concatenate(xs, 0)

        _, _, h, w = x.shape
        x = x.reshape((rows, cols*(gen.T + 3), 3, h, w))
        x = x.transpose(0, 3, 1, 4, 2)
        x = x.reshape((rows * h, cols*(gen.T + 3) * w, 3))
        x = np.asarray(np.clip(x * 127.5 + 127.5, 0.0, 255.0), dtype=np.uint8)
