...
from source.links.spatial_categorical_conditional_batch_normalization import SpatialCategoricalConditionalBatchNormalization
...
class Block(chainer.Chain):
    ...
    def residual(self, x, y=None, z=None, z1=None, z2=None, **kwargs):
        h = x
...
class SCBNBlock(chainer.Chain):
    def __init__(self, in_channels, out_channels, hidden_channels=None, ksize=3, pad=1,
                 activation=F.relu, upsample=False, n_classes=0):
        super(SCBNBlock, self).__init__()
        initializer = chainer.initializers.GlorotUniform(math.sqrt(2))
        initializer_sc = chainer.initializers.GlorotUniform()
        self.activation = activation
        self.upsample = upsample
        ...
        with self.init_scope():
            self.c1 = L.Convolution2D(in_channels, hidden_channels, ksize=ksize, pad=pad, initialW=initializer)
            self.c2 = L.Convolution2D(hidden_channels, out_channels, ksize=ksize, pad=pad, initialW=initializer)
            self.b1 = SpatialCategoricalConditionalBatchNormalization(in_channels, n_cat=n_classes)
            self.b2 = SpatialCategoricalConditionalBatchNormalization(hidden_channels, n_cat=n_classes)
            if self.learnable_sc:
                self.c_sc = L.Convolution2D(in_channels, out_channels, ksize=1, pad=0, initialW=initializer_sc)

    def residual(self, x, weights1=None, weights2=None, **kwargs):
        h = x
        h = self.b1(h, weights=weights1, **kwargs)
        h = self.activation(h)
        h = upsample_conv(h, self.c1) if self.upsample else self.c1(h)
        h = self.b2(h, weights=weights2, **kwargs)
        h = self.activation(h)
        h = self.c2(h)
        return h
