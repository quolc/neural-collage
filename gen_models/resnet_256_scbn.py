...
class SCBNResNetGenerator(chainer.Chain):
    def __init__(self, ch=64, dim_z=128, bottom_width=4, activation=F.relu, n_classes=0, distribution="normal"):
        ...
        with self.init_scope():
            ...
            self.block2 = SCBNBlock(ch * 16, ch * 16, activation=activation, upsample=True, n_classes=n_classes)
            self.block3 = SCBNBlock(ch * 16, ch * 8, activation=activation, upsample=True, n_classes=n_classes)
            self.block4 = SCBNBlock(ch * 8, ch * 8, activation=activation, upsample=True, n_classes=n_classes)
            self.block5 = SCBNBlock(ch * 8, ch * 4, activation=activation, upsample=True, n_classes=n_classes)
            self.block6 = SCBNBlock(ch * 4, ch * 2, activation=activation, upsample=True, n_classes=n_classes)
            self.block7 = SCBNBlock(ch * 2, ch, activation=activation, upsample=True, n_classes=n_classes)
    ...
    # blending weights: blends[DATA_ID][H][W][INPUT_ID]
    def spatial_interpolation(self, z=None, weights=None, zs=None, blends=None, **kwargs):
        if zs is None: # no feature blending
            ...
            h = self.block7(h, weights1=weights[10], weights2=weights[11], **kwargs)
            h = self.b8(h)
            h = self.activation(h)
            h = F.tanh(self.l8(h))
            return h
            ...
            for h in hs:
                ...
                _hs.append(h)
            hs = _hs
            hs[-1] = self.blend_featuremap(F.stack(hs, axis=1), blends[6])
